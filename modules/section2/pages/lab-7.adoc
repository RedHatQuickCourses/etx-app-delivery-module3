...
----
# Get cluster domain from the existing Keycloak route
CLUSTER_DOMAIN="$(oc get route -n keycloak \
  -o jsonpath='{.items[0].spec.host}' | sed 's/sso\.apps\.//')"
...
----
+
.Sample Argo CD Route
image::argo.png[width=500]
+
If the route does not exist, create it manually:
+
[source,bash,role=execute]
----
