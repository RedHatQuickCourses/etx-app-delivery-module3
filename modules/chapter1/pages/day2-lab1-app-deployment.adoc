...
. Select the **ETX Keycloak login** option
+
...
. Navigate to your openshfit console terminal and log into GitLab (get the URL first):
+
[source,bash,role=execute]
----
# Get GitLab URL
...
You can even navigate to argocd server and check the status of application
...
Click on the application to view more details

image::app_deploy[width=500]
...
Get the GitLab URL first, then update the pipeline:
...
Add the `update-gitops` task to the pipeline spec. 
...
Task definition to add, please note that it is suppose to be added under **tasks** section:
