= Lab 4: GitLab Integration and Shared Secrets
:navtitle: Lab 4: GitLab Integration & Shared Secrets
...
. Open {gitlab_url}[GitLab^] and authenticate with your SSO credentials.
. Click your avatar in the upper-right corner.
. Select menu:Edit profile[Access > Applications] to open the User Settings applications page.
+
...
WARNING: Replace `GITLAB_APPLICATION_ID` and `GITLAB_APPLICATION_SECRET` with the values from the GitLab OAuth application before running the command.
...
[#_example_ai_code_assistant_continue_credentials]
...
WARNING: Replace `REPLACE_API_KEY` with your actual API key from the MaaS platform before running the command.
