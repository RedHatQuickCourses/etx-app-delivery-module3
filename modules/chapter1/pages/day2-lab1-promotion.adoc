...
* Access to Quay via Keycloak SSO (`platform-admin` or `platform-developer`  / `openshift`)
...
Edit the configuration section with your cluster credentials (from RHDP emails), substitute XXXXX, {openshift_cluster_admin_password} with your lab GUID and lab password :

[source,bash,subs="attributes+"]
----
export RUNTIME_API="https://api.cluster-xxxxx.dyn.redhatworkshops.io:6443"
export RUNTIME_USER="admin"
export RUNTIME_PASSWORD="{openshift_cluster_admin_password}"

export FACTORY_API="https://api.cluster-xxxxx.dyn.redhatworkshops.io:6443"
...
You will notice output as below

[source,bash,role=execute]
----
[STEP] Step 1: Preparing runtime cluster for ArgoCD management
[INFO] Logging into runtime cluster: https://api.cluster-mp56f.dyn.redhatworkshops.io:6443
...
Warning: resource subscriptions/amq-streams is missing the kubectl.kubernetes.io/last-applied-configuration annotation which is required by oc apply. oc apply should only be used on resources created declaratively by either oc create --save-config or oc apply. The missing annotation will be patched automatically.
subscription.operators.coreos.com/amq-streams configured
[INFO] Installing OpenShift Pipelines (Tekton) operator...
Warning: resource subscriptions/openshift-pipelines-operator-rh is missing the kubectl.kubernetes.io/last-applied-configuration annotation which is required by oc apply. oc apply should only be used on resources created declaratively by either oc create --save-config or oc apply. The missing annotation will be patched automatically.
subscription.operators.coreos.com/openshift-pipelines-operator-rh configured
...
secret/cluster-staging created
...
Runtime cluster:  https://api.cluster-mp56f.dyn.redhatworkshops.io:6443
...
. Open link:{quay_url}[Quay] or click the *Quay* tab in your Showroom
. Sign in with Keycloak SSO using `platform-admin` or `platform-developer`  / `openshift`
...
+
image::org.png[width=400]
+
. Set the organization name to `etx_app` and click btn:[Create Organization]
+
image::create_org.png[width=400]
+
. Inside the `etx_app` organization, click btn:[Create New Repository]
+
image::repo1.png[width=400]
+
...
+
image::public.png[width=500]

NOTE: The organization and repository names must match the destination used later in the promotion pipeline: `etx_app/etx-app-base-app`.
...
. In the `etx_app` organization, click the *Settings* gear on the left panel
+
image::setting.png[width=500]
+
...
+
image::robot.png[width=500]
+
. Name the robot `promoter` and click btn:[Create Robot Account]
+
image::promoter.png[width=500]
+
. Grant the robot *Write* permission by change the `Read` dropdown to *Write* and click btn:[Add Permission]
+
image::write1.png[width=500]
+
...
. Open link:{vault_url}[Vault] or click the *Vault* tab in your Showroom
